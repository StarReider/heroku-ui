function updateMultiplication() {
	$.ajax({
		url : process.env.GATEWAY_SERVER_URL + "/multiplications/random"
	}).then(function(data) {
		// Cleans the form
...
function updateResults(alias) {
	...
	$.ajax({
		async : false,
		url : process.env.GATEWAY_SERVER_URL + "/results?alias=" + alias,
...
$(document).ready(function() {
	...
	$("#attempt-form").submit(function(event) {
		...
		$.ajax({
			url : process.env.GATEWAY_SERVER_URL + '/results',
			type : 'POST',
